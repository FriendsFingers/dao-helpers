...
class Dapp {
  ...
  formatStructure (struct) {
    ...
    return {
      ...
      address: struct[1],
      fingerprint: Dapp.formatFingerprint(struct[2]),
      creationDate: struct[3].valueOf() * 1000,
      stakedTokens: parseFloat(this.web3.fromWei(struct[4])),
      usedTokens: parseFloat(this.web3.fromWei(struct[5])),
      data: struct[6],
      approved: struct[7],
    };
  };
